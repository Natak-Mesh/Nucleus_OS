...
- [x] Stage 6: Meshtastic RX (live radio) — Code at `opt/nucleus/meshtastic/cot_bridge_rx.py`. Received ATAK_FORWARDER (portnum 257) from Node B (`!63ad2aba`) over LoRa: 111B wire payload, SNR=12.25dB, RSSI=-35dBm. Full RX pipeline verified: `TakCompressor.decompress()` → `CotXmlBuilder.build()` (585B CoT XML) → `xml2proto()` (213B TAK Protocol V1) → multicast inject to `239.2.3.1:6969`. Recovered PLI: uid=ANDROID-tx-test-node, callsign=TX-TEST, team=Cyan, lat=37.4154, lon=-77.635. Note: original "Python API doesn't receive remote packets" bug was caused by radio entering bad state after repeated serial connect/disconnect cycles — radio reboot resolved it; the SDK works correctly.
- [x] Stage 7: Bridge service — Code at `opt/nucleus/meshtastic/cot_bridge.py`. Standalone bidirectional daemon: TX listens on SA multicast (`239.2.3.1:6969`) and Chat multicast (`224.10.10.1:17012`), compresses to TAKPacketV2, sends as ATAK_FORWARDER (portnum 257) over LoRa with 30s per-UID rate limiting. RX receives ATAK_FORWARDER from LoRa, decompresses, converts to TAK Protocol V1, injects to appropriate multicast group on br-lan. Loop prevention via RX UID tracking. Both TX and RX verified bidirectionally between two nodes.
